...
float select(float y, float ys, float range) {
    y -= ys;
    y = abs(y);
    y = clamp(y, 0., range);
    y = range - y;
...
void main() {
	...
	float color = min_d;
    float s_color0 = select(color, 0.050, 0.028);
    float s_color1 = select(color, 0.100, 0.028);
    float s_color2 = select(color, 0.200, 0.028);
    float s_color3 = select(color, 0.400, 0.028);
    
    gl_FragColor = vec4(max(max(max(s_color0, s_color1), s_color2), s_color3));
}